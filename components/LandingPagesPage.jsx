...
const LandingPagesPage = ({ propertyId, dates, compareMode, prevDates }) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 backdrop-blur-sm bg-white/70 border border-white/20 shadow-xl p-4 md:p-6 rounded-2xl">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
        <h2 className="text-sm font-black uppercase text-slate-400 tracking-widest italic">Top Landing Pages</h2>
        <span className="text-[10px] bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-bold self-start sm:self-auto">Prime Pagine della Sessione</span>
      </div>
            ...
            {pages.map((page, i) => {
              ...
              return (
                <tr key={i} className="hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-3 font-bold text-blue-600 truncate max-w-[160px] sm:max-w-xs md:max-w-md">{path}</td>
                  <td className="px-4 py-3 text-right font-black text-slate-700">{page.sessions.toLocaleString('it-IT')}</td>
                  {compareMode && <td className="px-4 py-3 text-right">{deltaSessions ? <span className={`font-black ${deltaSessions.isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>{deltaSessions.formatted}</span> : '-'}</td>}
                  <td className="px-4 py-3 text-right font-bold text-slate-400">{page.users.toLocaleString('it-IT')}</td>
                  {compareMode && <td className="px-4 py-3 text-right">{deltaUsers ? <span className={`font-black ${deltaUsers.isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>{deltaUsers.formatted}</span> : '-'}</td>}
